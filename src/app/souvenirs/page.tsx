...
export default function SouvenirsPage() {
  ...
  useEffect(() => {
    ...
    const categoryParam = selectedCategory
      ...
      : "";
    fetch(
      `http://localhost:8080/souvenirs?page=${page - 1}&size=6${categoryParam}`
    )
      .then((res) => {
      ...
      .then((data) => {
        // Handle categories from the response (availableCategories)
        if (
          data.availableCategories &&
          Array.isArray(data.availableCategories)
        ) {
          const catNames = data.availableCategories.map((c: any) =>
            typeof c === "string" ? c : c.name
          );
          setCategories((prev) => (prev.length === 0 ? catNames : catNames));
        } else if (data.categories && Array.isArray(data.categories)) {
          // Fallback if structured differently in future
          ...
          {products.map((product, idx) => {
            ...
            return (
              <ProductPreview
                ...
                slug={product.slug}
                basePath="/souvenirs" // Added this prop
              />
            );
          })}
        </div>
